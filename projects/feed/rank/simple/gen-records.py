...
def build_features(infile):
  ...
  with melt.tfrecords.Writer(ofile) as writer:
    for line in tqdm(open(infile), total=total):
      fields = line.rstrip().split('\t')
      if len(fields) > 4:
        label = int(fields[0])
        id = '{}\t{}'.format(fields[2], fields[3])
        feat_id, feat_field, feat_value, doc_emb, user_emb = dataset.get_feat(fields[4:])
        ...
        feature = {
                    ...
                    'doc_emb': melt.float_feature(doc_emb),
                  }
