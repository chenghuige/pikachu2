...
if (($interval == 1))
then
pre_hour=`date -d"${valid_hour:0:8} ${valid_hour:8:10} -24hours" +"%Y%m%d%H"`
pre_hour2=`date -d"${valid_hour:0:8} ${valid_hour:8:10} -23hours" +"%Y%m%d%H"`
train_dir=$train_dir,$DIR/tfrecords/$pre_hour,$DIR/tfrecords/$pre_hour2
fi
