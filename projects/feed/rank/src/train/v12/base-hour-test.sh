...
ts_hour=`date -d"${start_hour:0:8} ${start_hour:8:10} -${interval}hours" +"%Y%m%d%H"`
ts_hour2=`date -d"${start_hour:0:8} ${start_hour:8:10} -${interval2}hours" +"%Y%m%d%H"`

train_dir=$DIR/$ts_hour/tfrecords
valid_dir=$DIR/$start_hour/tfrecords
model_dir=$DIR/$ts_hour2

echo $train_dir 
echo $valid_dir 

model=WideDeep
EVFIRST=0 python ./train.py \
    --save_model=0 \
