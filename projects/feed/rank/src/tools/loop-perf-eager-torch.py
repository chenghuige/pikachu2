...
from melt.eager.train import to_torch

# v2版本和v1相比慢很多 主要是慢在torch转换(多了很多分散的sparse2dense输入) 但是v1版本自身torch转换占用时间还ok
...
def main(_):  
  ...
  l = []
  for i, (x, y) in tqdm(enumerate(batches), total=num_steps, ascii=True, desc='loop'):
    x, y = to_torch(x, y)
    l.append(x['id'])
