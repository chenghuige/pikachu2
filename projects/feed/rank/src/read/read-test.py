...
def main(_):
  ...
  da = dataset.make_batch(4, repeat=False, shuffle=True)

  # #as for melt eager not repeat mode will get same sequence each epoch if seed is the same
  for epoch in range(2):
    for i, batch in enumerate(da):
      print(epoch, i, batch[0]['id'], batch[0]['id'].shape, batch[1].shape, batch[0]['index'].shape)
  ...
  print('-------------------------------------------------------------------')  
  da = dataset.make_batch(4, repeat=True, shuffle=True, num_epochs=2)
  for i, batch in enumerate(da):
    print(epoch, i, batch[0]['id'], batch[0]['id'].shape, batch[1].shape, batch[0]['index'].shape) 
  # for i, batch in enumerate(da):
  #     print('---------------------------', i)
