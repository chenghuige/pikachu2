...
def parse(line, tag):
  ...
  try:
    ...
    other = '\t'.join(l[6:])
    r = [key, (click, product, info, other, tag, impression_time)]
    return r
  except Exception:
    return None

d = d.map(lambda x: parse(x, 0)).filter(lambda x: x is not None)
d2 = d2.map(lambda x: parse(x, 1)).filter(lambda x: x is not None)
d = d.union(d2)
...
def dedup(l):
  ...
  # sort by impression time
  items = sorted(l[1], key=lambda x: (x[0], x[-2], x[-1]))
  for item in items:
    click, product_, info, other_, tag, _ = item
    if click == '1':
      clicked = '1'
      click_info = info
  ...
  # this show will leave in next run
  if show_tag != 0:
    return ''
