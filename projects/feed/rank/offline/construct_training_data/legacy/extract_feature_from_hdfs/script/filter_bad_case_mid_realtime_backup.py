...
def extract_sepcific_cycle_profile(cycle_name, feature_account):
    ...
    for key, val in topic.iteritems():
        topic_tuple_list.append((str(key), int(val[0]), int(val[1]), float(val[2])))
    topic_list = get_top_n(MAX_TOPIC_SIZE, topic_tuple_list)
    for i in range(len(topic_list)):
        topic_list[i] = cycle_name+"_topic\a"+str(topic_list[i])

    tag_tuple_list = list()
    for key, val in tag.iteritems():
        tag_tuple_list.append((str(key), int(val[0]), int(val[1]), float(val[2])))
    tag_list = get_top_n(MAX_TAG_SIZE, tag_tuple_list)
    for i in range(len(tag_list)):
        tag_list[i] = cycle_name+"_tag\a"+str(tag_list[i])

    kw_tuple_list = list()
    for key, val in kw.iteritems():
        kw_tuple_list.append((str(key), int(val[0]), int(val[1]), float(val[2])))
    kw_list = get_top_n(MAX_KW_SIZE, kw_tuple_list)
    for i in range(len(kw_list)):
        kw_list[i] = cycle_name+"_kw\a"+str(kw_list[i])

    topic_kw_tuple_list = list()
    for key, val in topic_kw.iteritems():
        topic_kw_tuple_list.append((str(key), int(val[0]), int(val[1]), float(val[2])))
    topic_kw_list = get_top_n(MAX_TOPIC_KW_SIZE, topic_kw_tuple_list)
    for i in range(len(topic_kw_list)):
        topic_kw_list[i] = cycle_name+"_txk\a"+str(topic_kw_list[i])

    account_tuple_list = list()
    for key, val in account.iteritems():
        account_tuple_list.append((str(key), int(val[0]), int(val[1]), float(val[2])))
    account_list = get_top_n(MAX_ACCOUNT_SIZE, account_tuple_list)
    for i in range(len(account_list)):
        account_list[i] = cycle_name+"_acc\a"+str(account_list[i])
...
def main():
    ...
    for line in sys.stdin:
	    ...
	    #    continue    
            ...
            #    if str(dur) == "None" and line_tuple[0] == "1":
            #        continue
            #    if dur > cur_door:
            #        line_tuple[0] = "1"
            #    else:
	    ...
	    #    continue
            ...
            cycle_profile_dur = ""
            if feature_account:
                account_list, topic_list, tag_list, kw_list, topic_kw_list = extract_sepcific_cycle_profile("long_term", feature_account)
                ...
                account_list, topic_list, tag_list, kw_list, topic_kw_list = extract_sepcific_cycle_profile("last_day", feature_account)
                cycle_profile_id += "\t"+"\t".join(account_list) +"\t"+"\t".join(topic_list)+"\t"+"\t".join(tag_list)+"\t"+"\t".join(kw_list)+"\t"+"\t".join(topic_kw_list)

                account_list, topic_list, tag_list, kw_list, topic_kw_list = extract_sepcific_cycle_profile("last_seven_day", feature_account)
                cycle_profile_id += "\t"+"\t".join(account_list) +"\t"+"\t".join(topic_list)+"\t"+"\t".join(tag_list)+"\t"+"\t".join(kw_list)+"\t"+"\t".join(topic_kw_list)

                account_list, topic_list, tag_list, kw_list, topic_kw_list = extract_sepcific_cycle_profile("last_three_day", feature_account)
                cycle_profile_id += "\t"+"\t".join(account_list) +"\t"+"\t".join(topic_list)+"\t"+"\t".join(tag_list)+"\t"+"\t".join(kw_list)+"\t"+"\t".join(topic_kw_list)
	    ...
	    if "lr" in model_name:
            ...
            line = "\t".join(line_tuple[0:4]) + "\t" + unlike + "\t" + all_interest_cnt + "\t" + ty + "\t" + dur + "\t" + "\t".join(line_tuple[5:]) + "\t" + feature_q_score + "\t"+ feature_w_cnt + "\t" + feature_p_cnt + "\t" + feature_p_score + "\t" + feature_avg_dur
            if profile_flag:
                line += "\t" + feature_sex + "\t" + feature_edu + "\t" + feature_age + "\t" + feature_favor_num
