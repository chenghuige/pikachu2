...
del_hour=`date -d"${start_hour:0:8} ${start_hour:8:10} -50hours" +"%Y%m%d%H"`
...
if [ ${model_name} = "" ];
then
    model_name="nfm_realtime"
fi
if [ ${model_name} = "sgsapp_kw" ];
then
    sample_input_dir="/user/traffic_dm/fujinbing/combined_feature_output_new" #mod by lyq
    #sample_input_dir="/user/traffic_dm/fujinbing/combined_feature_output_new_interest/"
fi
...
base_feature_merge="/user/traffic_dm/liuyaqiong/combined_feature_output/${start_hour}/${start_hour}_${interval}"
...
combined_feature_output=${base_feature_merge}
...
source "./run_construct_feature_nfm_function.sh"
source "./run_construct_feature_nfm_cmd_realtime.sh"
