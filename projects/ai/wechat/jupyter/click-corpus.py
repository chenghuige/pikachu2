...
def _pad(hist, days, action, day):
  hist = gezi.pad(hist, HIST_LENS[action])
  spans = [day - x + 1 for x in days]
  spans = gezi.pad(spans, HIST_LENS[action])
#   if FLAGS.his_cls:
#     hist.insert(0, EMPTY_ID)
#     spans.insert(0, EMPTY_ID)  
  return hist, spans
  ...
  if day > 1:
    ...
    for feedid_, day_ in hist:
      if day_ < day:
        ...
        days.append(day_)
  if not DYNAMIC_LEN:
    return _pad(feeds, days, action, day)
  else:
    return feeds[:HIST_LENS[action]], [day - x + 1 for x in days][:HIST_LENS[action]]
