...
class Dataset(melt.tfrecords.Dataset):
  def __init__(self, subset='train'):
    ...
    def get_aug_factor():
      ...
      return tf.cond(pred=tf.equal(tf.cast(global_step / num_examples, dtype=tf.int64) % 2, 0), true_fn=lambda: 0., false_fn=lambda: 1.)

    def undersampling_filter(x, y):
      prob = tf.cond(pred=tf.equal(x['source'], 'train'), true_fn=lambda: 1., false_fn=lambda: FLAGS.other_corpus_factor)
  ...
  def parse(self, example):
    ...
    features_dict = {
      ...
      'content_str': tf.io.FixedLenFeature([], tf.string),
      'content': tf.io.VarLenFeature(tf.int64),
      'char': tf.io.VarLenFeature(tf.int64),
      'pos': tf.io.VarLenFeature(tf.int64),
      'ner': tf.io.VarLenFeature(tf.int64),
      'wlen': tf.io.VarLenFeature(tf.int64),
      #'label': tf.FixedLenFeature([NUM_ATTRIBUTES], tf.int64),
      'source':  tf.io.FixedLenFeature([], tf.string),
      }

    if FLAGS.use_soft_label:
      features_dict['label'] =  tf.io.FixedLenFeature([NUM_ATTRIBUTES * NUM_CLASSES], tf.float32)
    else:
      features_dict['label'] =  tf.io.FixedLenFeature([NUM_ATTRIBUTES], tf.int64)
    ...
    if not FLAGS.use_soft_label:
      ...
      if FLAGS.binary_class_index is not None:
        y = tf.cast(tf.equal(y, FLAGS.binary_class_index), dtype=tf.int64)
    else:
      y = label
