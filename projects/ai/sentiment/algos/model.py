...
class ModelBase(melt.Model):
  ...
  def unk_aug(self, x, x_mask=None, training=False):
    ...
    def aug(x, x_mask):
      ...
      ratio = tf.random.uniform([1,], 0, FLAGS.unk_aug_max_ratio)
    ...
    return tf.cond(pred=tf.compat.v1.train.get_or_create_global_step() < tf.constant(FLAGS.unk_aug_start_step, dtype=tf.int64), true_fn=lambda: x, false_fn=lambda: aug(x, x_mask))
