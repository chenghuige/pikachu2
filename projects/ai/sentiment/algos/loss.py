...
def calc_loss(y, y_, weights, training=False):
  ...
  if training and FLAGS.num_learning_rate_weights == NUM_ATTRIBUTES:
    ...
    loss = tf.compat.v1.losses.sparse_softmax_cross_entropy(logits=y_, labels=y, weights=weights, reduction=tf.compat.v1.losses.Reduction.NONE)
    ...
    if FLAGS.loss == 'cross':
      if not FLAGS.label_smoothing:
        loss = tf.compat.v1.losses.sparse_softmax_cross_entropy(y, y_, weights=weights) 
      else:
        onehot_labels = tf.one_hot(y, NUM_CLASSES)
        print('--------------using label smoothing', FLAGS.label_smoothing)
        loss = tf.compat.v1.losses.softmax_cross_entropy(onehot_labels, y_, weights=weights, label_smoothing=FLAGS.label_smoothing)
    elif FLAGS.loss == 'focal':
      loss = melt.losses.focal_loss(y, y_)

  #----------depreciated below has bug..
  if FLAGS.na_loss_ratio > 0.:
    y_ = tf.concat([y_[:,:,0:1], tf.reduce_sum(input_tensor=y_[:,:,1:], axis=-1, keepdims=True)], -1)
    y_onehot = tf.one_hot(tf.cast(y > 0, dtype=tf.int64), 2)
    if no_weights():
      bloss = tf.compat.v1.losses.sigmoid_cross_entropy(y_onehot, y_)
    else:
      bloss = tf.compat.v1.losses.sigmoid_cross_entropy(y_onehot, y_, reduction=tf.compat.v1.losses.Reduction.NONE)
      bloss = tf.reduce_sum(input_tensor=bloss * tf.expand_dims(weights, -1), axis=-1)
      bloss = tf.reduce_mean(input_tensor=bloss)
...
def calc_hier_loss(y, y_, weights):
  binary_label = tf.cast(tf.equal(y, 0), dtype=tf.int64)
  # sigmoid reduction by default is not None will return scalar and if set None will return result shape as label
  level0_loss  = tf.compat.v1.losses.sigmoid_cross_entropy(binary_label, y_[:,:,0], weights=weights, reduction=tf.compat.v1.losses.Reduction.NONE)
  mask = tf.cast(1 - binary_label, dtype=tf.float32)
  # softmax loss reduction by defualt is not None and will return scalar, set None will return shape as label
  level1_loss = tf.compat.v1.losses.sparse_softmax_cross_entropy(tf.maximum(y - 1, 0), y_[:,:,1:], weights=weights, reduction=tf.compat.v1.losses.Reduction.NONE)
...
def calc_hier_neu_loss(y, y_, weights):
  ...
  # add neu binary
  cid = 2
  binary_label = tf.cast(tf.equal(y, cid), dtype=tf.int64)
  binary_loss = tf.compat.v1.losses.sigmoid_cross_entropy(binary_label, y_[:,:,cid], weights=weights, reduction=tf.compat.v1.losses.Reduction.NONE)
...
def calc_add_binary_loss(y, y_, cids, weights):
  reduction = tf.compat.v1.losses.Reduction.SUM_BY_NONZERO_WEIGHTS if FLAGS.loss_combine_by_scalar else tf.compat.v1.losses.Reduction.NONE
  loss = tf.compat.v1.losses.sparse_softmax_cross_entropy(logits=y_, labels=y, weights=weights, reduction=reduction) 
  for cid in cids:
    binary_label = tf.cast(tf.equal(y, cid), dtype=tf.int64)
    binary_loss = tf.compat.v1.losses.sigmoid_cross_entropy(binary_label, y_[:,:,cid], weights=weights, reduction=reduction)
...
def calc_binary_loss(y, y_, cid, weights):
  binary_label = tf.cast(tf.equal(y, cid), dtype=tf.int64)
  binary_loss = tf.compat.v1.losses.sigmoid_cross_entropy(binary_label, y_[:,:,cid], weights=weights)
  return binary_loss

def calc_regression_loss(y, y_, weights):
  y = y * 2 + 2
  return tf.compat.v1.losses.mean_squared_error(y, y_, weights=weights)
...
def criterion(y, y_):
  ...
  mask = y >= 0
  weights = weights * tf.cast(mask, dtype=tf.float32)
  y = y * tf.cast(mask, dtype=tf.int64)
  ...
  if FLAGS.loss_type == 'normal':
    ...
    return tf.compat.v1.losses.sigmoid_cross_entropy(tf.cast(y[:,FLAGS.binary_class_index:FLAGS.binary_class_index + 1] > 0, dtype=tf.int64), y_, 
                                           reduction=tf.compat.v1.losses.Reduction.NONE, weights=weights)
