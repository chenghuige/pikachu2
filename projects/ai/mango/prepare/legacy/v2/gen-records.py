...
def deal(index):
  ...
  with melt.tfrecords.Writer(ofile) as writer:
    for _, row in tqdm(df_.iterrows(), total=len(df_), ascii=True):      
      ...
      feature['first_stars_list'] = row['first_stars_list']
...
def main(_):  
  ...
  print('--------loading user')
  user_file = f'../input/{FLAGS.mark}/part_{FLAGS.day}/user.parquet' if FLAGS.mark == 'train' else f'../input/{FLAGS.mark}/user.parquet'
# if not os.path.exists(user_file.replace('.parquet', '.csv')):
  ...
  #   print('-------dump modified user file to disk')
  #   udf.to_csv(user_file.replace('.parquet', '.csv'), index=False)
  # else:
