...
# from gseg.third.ghostnet_tf2.ghost_model import GhostNet
from gseg.third.ghostnet_keras.model.GhostNet import GhostNet
...
class BackbonesFactory(ModelsFactory):
    _default_feature_layers = {
        ...
        #ghostnet
        'ghostnet': ('activation_27', 'activation_16', 'activation_9', 'activation_6'),
    ...
    _models_update = {
        ...
        'ghostnet': [GhostNet, eff.preprocess_input],
    }
    ...
    def get_feature_layers(self, name, n=5):
        if gezi.get('EFFDET_FEATS') and name.startswith('efficientnet'):
            # 16, 32, 64, 128 但是这个unet实验效果不如原版的backbone layers TODO EffDet换用block6a_expand_activation 系列？
            feats = ('block5c_add', 'block3b_add', 'block2b_add', 'block1a_project_bn')
            logging.info(f'get {name} as effdet:', feats)
            return feats[:n] 
