...
def _show(imgs, tag, i, logger, step):
  ...
  prob =imgs[6]
  prob_label = imgs[7]
  # logger.images(f'{tag}/{i}', [img, label, pred_], step, title=text) 
  # try:
  # img_ = plot.segmentation_eval_logits(img, label, logit, FLAGS.CLASSES, preds=pred, show=FLAGS.show_imgs_online, img2bytes=False)
  img_ = plot.segmentation_eval(img, label, pred, FLAGS.CLASSES, prob, prob_label, title=text)
  # except Exception as e:
  #   logging.error(e)
  #   logging.error('text', text)
  #   logging.error('label', label, label.shape)
  if not FLAGS.wandb or FLAGS.wandb_tb:
    logger.image(f'{tag}/{i}', gezi.plot.tobytes(img_), step, title=text)
  else:
    # TODO wandb 写入不显示 暂时选择sync tensorboard
    ## 如果输入是iobytes也就是img通过gezi.plot.tobytes转换之后的 写tensorboard可以直接写 写wandb需要numpy array或者 PIL image
    # img_ = np.array(Image.open(img_))
  ...
  def __call__(self, image_id, image, label, y_pred):
    ...
    if self.display_results:
      ...
      if self.step in self.random_img_indexes:
        _show(item, '1Random', list(self.random_img_indexes).index(self.step), self.logger, self.step)
