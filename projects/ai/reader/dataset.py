...
class Dataset(melt.tfrecords.Dataset):
  def __init__(self, subset='train'):
    ...
    if FLAGS.type1_count:
      self.count_fn = lambda x, y: tf.cond(pred=tf.equal(x['type'], 1), true_fn=lambda: FLAGS.type1_count, false_fn=lambda: 1)
  ...
  def parser(self, example):
    features_dict = {
      ...
      'answer_str':  tf.io.FixedLenFeature([], tf.string),
      'query': tf.io.VarLenFeature(tf.int64),
      'query_char': tf.io.VarLenFeature(tf.int64),
      'query_pos': tf.io.VarLenFeature(tf.int64),
      'query_str':  tf.io.FixedLenFeature([], tf.string),
      'passage': tf.io.VarLenFeature(tf.int64),
      'passage_char': tf.io.VarLenFeature(tf.int64),
      'passage_pos': tf.io.VarLenFeature(tf.int64),
      'passage_str':  tf.io.FixedLenFeature([], tf.string),
      'candidate_neg':  tf.io.VarLenFeature(tf.int64),
      'candidate_neg_char': tf.io.VarLenFeature(tf.int64),
      'candidate_neg_pos': tf.io.VarLenFeature(tf.int64),
      'candidate_pos':  tf.io.VarLenFeature(tf.int64),
      'candidate_pos_char': tf.io.VarLenFeature(tf.int64),
      'candidate_pos_pos': tf.io.VarLenFeature(tf.int64),
      'candidate_na': tf.io.VarLenFeature(tf.int64),
      'candidate_na_char': tf.io.VarLenFeature(tf.int64),
      'candidate_na_pos': tf.io.VarLenFeature(tf.int64),
      'query_char': tf.io.VarLenFeature(tf.int64),
      'query_pos': tf.io.VarLenFeature(tf.int64),
      'alternatives':  tf.io.FixedLenFeature([], tf.string),
      'candidates':  tf.io.FixedLenFeature([], tf.string),
