...
def main(_):
    ...
    valid_dataset = nonenglish_val_datasets['Combined']

    import gezi
    from dataset import Dataset

    inputs = gezi.list_files('./output/tfrecords/jigsaw-toxic-comment-train-processed-seqlen128')
    train_dataset_ = Dataset('train')
    train_dataset = train_dataset_.make_batch(batch_size, inputs, repeat=True, cache=True, buffer_size=2048)

   
    valid_inputs = gezi.list_files('./output/tfrecords/validation-processed-seqlen128')
    valid_dataset_ = Dataset('valid')
    valid_dataset = valid_dataset_.make_batch(batch_size, valid_inputs, repeat=False, cache=True, buffer_size=2048)

    history = multilingual_bert.fit(
        train_dataset, 
        steps_per_epoch=4000/strategy.num_replicas_in_sync,
        epochs=5, verbose=1, 
