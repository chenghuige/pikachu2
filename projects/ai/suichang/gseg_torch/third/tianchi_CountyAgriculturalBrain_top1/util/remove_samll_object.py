...
if __name__ == "__main__":
    ...
    parser.add_argument("-image_n",type=int,help="传入5或6，指定")
    parser.add_argument("-image_path",type=str,help="传入image_n_predict所在路径")
    ...
    if image_n == 5:
        source_image = cv.imread("./output/source/image_5_vis.png")
        mask = np.asarray(Image.open("./output/source/image_5_mask.png"))
    elif image_n == 6:
        source_image = cv.imread("./output/source/image_6_vis.png")
        mask = np.asarray(Image.open("./output/source/image_6_mask.png"))
    else:
        raise ValueError("image_n should be 5 or 6, Got {} ".format(image_n))
