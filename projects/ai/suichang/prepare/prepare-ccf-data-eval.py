...
img_sizes = [256, 256, 260, 260, 288, 288, 288, 300, 400, 500, 500, 501, 512, 512, 533, 533, 533, 534, 535, 600, 811, 888, 900, 1023, 1025, 1055, 1055, 1055, 1056, 1057, 1500, 3000, 3100]
img_sizes = list(reversed(img_sizes))
...
if not osp.exists('../input/ccf_remote_dataset/eval/image'):
    os.makedirs('../input/ccf_remote_dataset/eval/image')
if not osp.exists('../input/ccf_remote_dataset/eval/label'):
    os.makedirs('../input/ccf_remote_dataset/eval/label')
...
#             left = w
#             upper = h
#             right = min(w + img_sizes[0], W)
#             lower = min(h + img_sizes[1], H)
...
#             cv2.imwrite("../input/ccf_remote_dataset/eval/image/{}_{}.png".format(
#                 train_id, train_tile_id), tile_image)
#             cut_label = label.crop((left, upper, right, lower))
#             cut_label.save("../input/ccf_remote_dataset/eval/label/{}_{}.png".format(
...
for h in range(0, H, val_stride[1]):
    for w in range(0, W, val_stride[0]):
        hws.append((h, w))
t = tqdm(enumerate(hws), total=len(hws), desc='sub_image')
np.random.seed(12345)
for i, (h, w) in t:
        left = w
        upper = h
        if i > len(img_sizes):
          np.random.shuffle(img_sizes)
        right = min(w + img_sizes[0], W)
        lower = min(h + img_sizes[0], H)
        cut_image = image[upper:lower, left:right, :]
        if cut_image.shape[:2] != (img_sizes[0], img_sizes[0]):
          continue
