...
def random_crop_resize(img,
  ...
  img_shape = (img.shape[0],
               img.shape[1]) if not resize_shape else resize_shape
  ...
  img = tf.reshape(img, [*img_shape, img.shape[-1]])
...
def augment_color_oneof(image):
  def _aug(image):
    n_ch = image.shape[-1]
    if n_ch > 3:
      image, other = tf.split(image, [3, n_ch - 3], axis=-1)
    p = tf.random.uniform(())
    if p <= (1 / 3.):
...
def mosaic_mix(image, mask, p=1.0):
  n_ch = image.shape[-1]
  ...
  def _deal(image, j, k, l, m):
    img1 = image[j, 0:index, 0:index, :]
    img2 = image[k, 0:index, index:, :]
    img3 = image[l, index:, 0:index, :]
    img4 = image[m, index:, index:, :]

    one = tf.concat([img1, img2], axis=1)
    two = tf.concat([img3, img4], axis=1)
    img = tf.concat([one, two], axis=0)
    return img
  ...
  for j in range(AUG_BATCH):
    ...
    l = tf.random.uniform([], 0, AUG_BATCH, dtype=tf.int32)
    m = tf.random.uniform([], 0, AUG_BATCH, dtype=tf.int32)
 
    # MAKE CUTMIX IMAGE
    img = _deal(image, j, k, l, m)
    imgs.append(img)
    mask_ = _deal(mask, j, k, l, m)
    masks.append(mask_)

  image2 = tf.stack(imgs, axis=0)
  mask2 = tf.stack(masks, axis=0)
  # RESHAPE HACK SO TPU COMPILER KNOWS SHAPE OF OUTPUT TENSOR (maybe use Python typing instead?)
  image2 = tf.reshape(image2, (AUG_BATCH, DIM, DIM, n_ch))
...
def cutmix(image, mask, pred=None, p=1.0):
  ...
  def _deal(image, j, k, xa, xb, ya, yb):
    ...
    img = tf.concat([image[j, 0:ya, :, :], middle, image[j, yb:DIM, :, :]],
                    axis=0)
...
def scaled_cutmix(image, mask, pred=None, p=1.0):
  ...
  for j in range(AUG_BATCH):
    ...
    masking = tf.cast(
        tf.random.uniform([]) <= FLAGS.cutmix_scale_rate, tf.int32)
...
def mixup(image, mask, p=1.0):
  ...
  for j in range(AUG_BATCH):
    ...
    img1 = image[j]
    img2 = image[k]
    a = tf.cast(a, img1.dtype)
    imgs.append((1 - a) * img1 + a * img2)
    # MAKE CUTMIX LABEL
    probs.append(a)
    mask_j = mask[j]
    mask_k = mask[k]
    mask_ = tf.concat([mask_j, mask_k], -1)
    masks.append(mask_)

  # RESHAPE HACK SO TPU COMPILER KNOWS SHAPE OF OUTPUT TENSOR (maybe use Python typing instead?)
  image2 = tf.reshape(tf.stack(imgs), (AUG_BATCH, DIM, DIM, n_ch))
  mask2 = tf.reshape(tf.stack(masks), (AUG_BATCH, DIM, DIM, 2))
  prob2 = tf.reshape(tf.stack(probs), (AUG_BATCH, 1))
  return image2, mask2, prob2

def mixup_transform(image, mask):
  n_ch = image.shape[-1]
  # input image - is a batch of images of size [n,dim,dim,3] not a single image of [dim,dim,3]
  # output - a batch of images with cutmix applied
  DIM = mt.get_shape(image, 1)
  ...
  SWITCH = FLAGS.mixup_switch
  CUTMIX_PROB = FLAGS.cutmix_rate
  cutmix_fn = cutmix if not FLAGS.cutmix_range else scaled_cutmix
  MIXUP_PROB = FLAGS.mixup_rate
  # FOR SWITCH PERCENT OF TIME WE DO CUTMIX AND (1-SWITCH) WE DO MIXUP
  image2, mask2 = cutmix_fn(image, mask, p=CUTMIX_PROB)
  mask2 = tf.concat([mask2, mask2], -1)
  image3, mask3, prob3 = mixup(image, mask, p=MIXUP_PROB)
  prob2 = tf.ones_like(prob3) * 0.5
  imgs = []
  masks = []
  probs = []
  for j in range(AUG_BATCH):
    P = tf.cast(tf.random.uniform([], 0, 1) <= SWITCH, image2[0].dtype)
    imgs.append(P * image2[j] + (1 - P) * image3[j])
    masks.append(P * mask2[j] + (1 - P) * mask3[j])
    probs.append(P * prob2[j] + (1 - P) * prob3[j])
  # RESHAPE HACK SO TPU COMPILER KNOWS SHAPE OF OUTPUT TENSOR (maybe use Python typing instead?)
  image4 = tf.reshape(tf.stack(imgs), (AUG_BATCH, DIM, DIM, n_ch))
  mask4 = tf.reshape(tf.stack(masks), (AUG_BATCH, DIM, DIM, 2))
  prob4 = tf.reshape(tf.stack(probs), (AUG_BATCH, 1))
  return image4, mask4, prob4

def mosaic_transform(image, mask):
  n_ch = image.shape[-1]
  # input image - is a batch of images of size [n,dim,dim,3] not a single image of [dim,dim,3]
  # output - a batch of images with cutmix applied
  DIM = mt.get_shape(image, 1)
  ...
  CUTMIX_PROB = FLAGS.cutmix_rate
  cutmix_fn = cutmix if not FLAGS.cutmix_range else scaled_cutmix
  MIXUP_PROB = FLAGS.mosaic_rate
  # FOR SWITCH PERCENT OF TIME WE DO CUTMIX AND (1-SWITCH) WE DO MIXUP
  image2, mask2 = cutmix_fn(image, mask, p=CUTMIX_PROB)
  image3, mask3 = mosaic_mix(image, mask, p=MIXUP_PROB)
  imgs = []
  masks = []
  for j in range(AUG_BATCH):
    P = tf.cast(tf.random.uniform([], 0, 1) <= SWITCH, image2[0].dtype)
...
def augment(image, mask=None, aug_mask=True):
  ...
  if FLAGS.augment_level == 0:
    image, mask = apply2(tf.image.flip_left_right,
                         ...
                         p=FLAGS.hflip_rate,
                         aug_mask=aug_mask)
    image, mask = apply2(tf.image.flip_up_down,
                         image,
                         mask,
                         p=FLAGS.hflip_rate,
                         aug_mask=aug_mask)
