...
def build_wBiFPN(features, num_channels, id, freeze_bn=False):
    if id == 0:
        ...
        P4_U = tf.image.resize(P4_td, P3_in.shape[1:3])
...
def build_BiFPN(features, num_channels, id, freeze_bn=False):
    if id == 0:
        ...
        P4_in_1 = layers.BatchNormalization(momentum=MOMENTUM, epsilon=EPSILON,
                                            name=f'fpn_cells/cell_{id}/fnode2/resample_0_1_7/bn')(P4_in_1)
        # P4_in_1 = BatchNormalization(freeze=freeze_bn, name=f'fpn_cells/cell_{id}/fnode2/resample_0_1_7/bn')(P4_in_1)
        P5_U = tf.image.resize(P5_td, P5_U.shape[1:3])
...
class SegmentationHead(tf.keras.layers.Layer):
  ...
  def __init__(self,
    ...
    if head_strategy == 'transpose':
        ...
        assert image_size
        self.head_process = tf.keras.layers.Lambda(lambda x: tf.image.resize(x, image_size))
        self.head_conv = tf.keras.layers.Conv2D(num_classes, kernel_size=(1,1), padding='same')
    else:
...
def efficientdet(phi, input_shape, num_classes=20, num_anchors=9, weighted_bifpn=False, freeze_bn=False,
    ...
    # TODO 这个微调的本地eff 载入noisy student权重是否有问题 效果？ 考虑还是用标准eff 获取相应layer ？
    ...
    seg_head = SegmentationHead(num_classes, filters, 3, 7, 'channels_last', not freeze_bn, 'swish', None, 
                                head_strategy=head_strategy, upsampling_last=upsampling_last, 
                                name='seg_head')
    outputs = seg_head(fpn_features, K.learning_phase())

    # print(outputs)
    # print(outputs.shape)
    
    # exit(0)
    model = models.Model(image_input, outputs, name='efficientdet')
    return model
