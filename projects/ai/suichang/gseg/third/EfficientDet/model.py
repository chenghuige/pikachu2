...
from .tfkeras import EfficientNetB3, EfficientNetB4, EfficientNetB5, EfficientNetB6, EfficientNetB7
...
class SegmentationHead(tf.keras.layers.Layer):
  ...
  def __init__(self,
    ...
    self.con2d_ts = []
    self.con2d_t_bns = []
    logging.debug('------------------------', min_level, max_level)
    ## 注意这里模仿unet多一次transpose 但是事实上和直接原有最后image resize * 2效果几乎一样 速度也差不多
    if not gezi.get('COMPAT_OLD') or not gezi.get('LESS_LEVELS'):
      max_level += 1  # chg 20201205 add just like unet
    logging.debug(gezi.get('COMPAT_OLD'), min_level, max_level)
  ...
  def get_config(self):
    ...
    # ------output (8, 32, 32, 768)
    # ------output (8, 64, 64, 768)
    # ------output (8, 128, 128, 384)
    # ------------final shape (8, 256, 256, 10)
  def call(self, feats, training=False):
    ...
    logging.debug('--------len(skips)', len(skips), len(self.con2d_t_bns))
    # print(skips)
...
def efficientdet(phi, input_shape, num_classes=20, num_anchors=9, weighted_bifpn=True, freeze_bn=False,
    ...
    logging.debug('--------backbone_features', features)
    ...
    if weighted_bifpn:
        ...
        for i in range(d_bifpn):
            ...
            logging.debug(i, w_bifpn, fpn_features)
    else:
        fpn_features = features
        for i in range(d_bifpn):
    ...
    logging.debug('----------1', feats)
    feats = feats[start_level:]
    logging.debug('---------2', start_level, feats)
