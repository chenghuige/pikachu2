...
class Model(mt.Model):
  def __init__(self, model, backbone, **kwargs):
    ...
    # if FLAGS.use_nir:
    #   self.r_dense = tf.keras.layers.Dense(1, activation='sigmoid')
    #   self.g_dense = tf.keras.layers.Dense(1, activation='sigmoid')
    #   self.b_dense = tf.keras.layers.Dense(1, activation='sigmoid')
  ...
  def call(self, input, training=False):
    ...
    if image.shape[-1] == 3:
      ...
      image1 = self.preprocess(image[:,:,:,:3])
      image2 = self.preprocess(image[:,:,:,1:])
      image = tf.concat([image1, image2[:,:,:,2:]], -1)
  ...
  def get_model(self):
    ...
    if not FLAGS.dynamic_image_size:
      img_input = Input(shape=(*FLAGS.ori_image_size, n_ch), name='image')    
    else:
      img_input = Input(shape=(None, None, n_ch), name='image')
