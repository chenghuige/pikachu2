...
def _to_onehot_label(y_true, y_pred):
  ...
  if y_true2 is not None:
    if len(y_true2.shape) == len(y_pred.shape):
      y_true2 = tf.squeeze(y_true2, -1)
    y_true2 = tf.one_hot(tf.cast(y_true2, tf.int32), y_pred.shape[-1])

    y_true = y_true * (1 - prob) + y_true2 * prob
...
#   r = 1 - (numerator + ep) / (denominator + ep)
...
def dsc(y_true, y_pred):
  ...
  score = (2. * intersection + smooth) / (K.sum(y_true_f + y_pred_f) + smooth)
  return score
