...
if __name__ == '__main__':
  ...
  for _, row in tqdm(df.iterrows(), total=len(df), ascii=True):
    for y_prob in row['y_probs']:
      impression_ids2 += [row['impression_id']]
      probs += [y_prob]

  for _, row in tqdm(df_truth.iterrows(), total=len(df), ascii=True):
    for y_true in row['y_trues']:
      impression_ids += [row['impression_id']]
      truths += [y_true]

      if len(truths) == len(probs):
        break

  for i in range(len(impression_ids)):
    assert impression_ids[i] == impression_ids2[i]
