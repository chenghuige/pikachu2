...
flags.DEFINE_bool('record_padded_', True, '')
...
flags.DEFINE_integer('max_abstracts', 50, '50 is better')
flags.DEFINE_integer('max_bert_abstracts', 50, '')
flags.DEFINE_integer('max_bodies', 50, '50 is better')
...
flags.DEFINE_string('pooling', 'att', '')
flags.DEFINE_string('feat_pooling', 'concat,dot', '')
...
flags.DEFINE_bool('mlp_bn', False, '')
flags.DEFINE_bool('mlp_ln', False, '')
...
flags.DEFINE_integer('l2_norm_axis', None, '')
...
flags.DEFINE_integer('min_did_count', 0, '')

flags.DEFINE_bool('rare_did_as_unk', False, '')
...
flags.DEFINE_bool('train_mask_dids', False, '')
flags.DEFINE_bool('valid_mask_dids', True, '')
flags.DEFINE_bool('test_mask_dids', True, 'a litter better 0.7123->0.7124')
flags.DEFINE_float('mask_dids_ratio', 0.92, 'need to do exp # dev new did ratio 0.054, test is 0.872')
...
flags.DEFINE_bool('use_multi_dropout', True, 'multiple dropout验证test有效 valloss也降低 设置为默认')
...
flags.DEFINE_bool('use_seq_position', False, '')

flags.DEFINE_bool('use_impression_titles', False, '')

flags.DEFINE_integer('max_bert_input_length', 30, '')

flags.DEFINE_bool('share_words_encoder', False, '')
flags.DEFINE_bool('bert_pooling_seqs', False, '')
flags.DEFINE_bool('bert_only', True, '')
flags.DEFINE_bool('bert_abstract', True, '')
flags.DEFINE_bool('bert_body', True, '')
flags.DEFINE_bool('bert_title_body', False, '')

flags.DEFINE_float('bert_dropout', 0., '')
flags.DEFINE_float('bert_lrrate', 1., '0.002 目前添加bert title效果最好 需要调试验证')
flags.DEFINE_bool('bert_trainable', True, '')
flags.DEFINE_bool('from_pt', True, '')

flags.DEFINE_bool('sort_result', False, '')
flags.DEFINE_bool('save_ori_result', True, '')
flags.DEFINE_string('prediction_file', 'prediction.csv', '')
...
def init():
  ...
  # 实验下面效果最好 HACK 强制固定, test的设置影响不大 test 设置在train出现保持did 没出现unk 稍微好一点点 7123->7124
  FLAGS.train_mask_dids = False
  FLAGS.test_mask_dids = True
  # FLAGS.mask_dids_ratio = 0.92
  FLAGS.test_all_mask = False

  # FLAGS.infer_write_streaming = True
  ...
  for vocab_name in vocab_names:
    ...
    if vocab_name == 'did' and FLAGS.min_did_count:
      vocab_size[1] = vocab.size(FLAGS.test_start + FLAGS.min_did_count)
