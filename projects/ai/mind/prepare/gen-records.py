...
def build_features(index):
  ...
  if FLAGS.neg_parts > 1:
    ...
    ofile = f'{FLAGS.out_dir}/{FLAGS.mark}-{FLAGS.neg_part}/record_{index}.tfrec'
  ...
  if FLAGS.mark == 'train' and FLAGS.train_by_day:
    ...
    for i in range(num_days):
      ...
      ofiles += [f'{FLAGS.out_dir}/{folder_name}-days/{i}/record_{index}.tfrec']
  ...
  for _, row in tqdm(df_.iterrows(), total=len(df_), ascii=True):
    ...
    # TODO 如此采样 对应neg_part越大的 采样到的负样本越少 另外 是否如果负样本本来比较少的时候不再做降采样
    if FLAGS.neg_parts > 1:
    ...
    for i, impression in enumerate(impressions):
      ...
      click = int(click)
      if not click:
        nonclick_indexes.append(i)
        if FLAGS.neg_parts > 0:
          if indexes[i] % FLAGS.neg_parts == FLAGS.neg_part:
            vindexes.append(i)

    vindexes_ = set(vindexes)

    if FLAGS.min_neg and FLAGS.neg_parts > 0:
      min_neg = min(FLAGS.min_neg, len(nonclick_indexes))
      if len(vindexes) < min_neg:
        for _ in range(FLAGS.neg_part + 1):
          np.random.shuffle(nonclick_indexes)
        for j in nonclick_indexes:
          vindexes_.add(j)
          if len(vindexes_) == min_neg:
            break          
    ...
    for i, impression in enumerate(impressions):
      ...
      if FLAGS.neg_parts > 0:
        if not click and i not in vindexes_:
          continue
...
def main(_):
  ...
  FLAGS.record_padded = FLAGS.record_padded_
...
if __name__ == '__main__':
  ...
  flags.DEFINE_integer('seed_', 20210226, '12345 padd,1024 padd3,2021 padd4, 20211021 padd5')
  flags.DEFINE_bool('shuffle_impressions', False, '')
  flags.DEFINE_bool('train_by_day', False, '')
  flags.DEFINE_string('record_name', 'tfrecords.v6', '')
  ...
  flags.DEFINE_integer('min_neg', None, '')
