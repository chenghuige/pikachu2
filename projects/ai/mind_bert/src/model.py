...
def bert_model(trainable_bert=True):
    ...
    segment_ids = tf.keras.layers.Input(
        shape=(max_seq_length,), dtype=tf.int64, name="all_segment_id")
...
def xlm_old_model():
  ...
  input_word_ids = tf.keras.Input(shape=(FLAGS.max_len,), dtype=tf.int64, name="input_word_ids")
...
def xlm_model():
  inputs = {
            'input_word_ids': Input(shape=(FLAGS.max_len,), dtype=tf.int64, name="input_word_ids"),
            }
  if FLAGS.use_word_ids2:
    inputs['input_word_ids2'] = Input(shape=(FLAGS.max_len,), dtype=tf.int64, name="input_word_ids2")
  out = XlmModel()(inputs)
