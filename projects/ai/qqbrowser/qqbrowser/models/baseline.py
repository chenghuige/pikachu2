...
class NeXtVLAD(tf.keras.layers.Layer):
    ...
    def call(self, inputs, **kwargs):
        ...
        if mask is not None:  # in case num of images is less than num_segments
            ...
            images_mask = tf.cast(tf.expand_dims(images_mask, -1), image_embeddings.dtype)
...
class MultiModal(mt.Model):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.input_ = None
        if not os.path.exists(FLAGS.transformer):
            self.bert = TFAutoModel.from_pretrained(FLAGS.transformer, from_pt=FLAGS.from_pt)
        else:
            config = BertConfig.from_json_file(f'{FLAGS.transformer}/bert_config.json')
            self.bert = TFAutoModel.from_pretrained(FLAGS.transformer, from_pt=FLAGS.from_pt, config=config)
        # self.bert = TFBertModel.from_pretrained(FLAGS.bert_dir)

        self.nextvlad = NeXtVLAD(FLAGS.frame_embedding_size, FLAGS.vlad_cluster_size,
                                 output_size=FLAGS.vlad_hidden_size, dropout=FLAGS.dropout)
        self.fusion = ConcatDenseSE(FLAGS.hidden_size, FLAGS.se_ratio)
        ...
        # self.bert_optimizer, self.bert_lr = create_optimizer(init_lr=FLAGS.bert_lr,
        #                                                      num_train_steps=config.bert_total_steps,
        #                                                      num_warmup_steps=config.bert_warmup_steps)
        # self.optimizer, self.lr = create_optimizer(init_lr=FLAGS.lr,
        #                                            num_train_steps=config.total_steps,
        #                                            num_warmup_steps=config.warmup_steps)
        
        bert_layers = [self.bert]
        base_layers = [layer for layer in self.layers if layer != self.bert]
        opt_layers = [base_layers, bert_layers]
        gezi.set('opt_layers', opt_layers)

    def call(self, inputs):
        self.input_ = inputs
        bert_embedding = self.bert([inputs['title_ids'], inputs['title_mask']])[1]
        frame_num = tf.reshape(inputs['num_frames'], [-1])
        vision_embedding = self.nextvlad([inputs['frames'], frame_num])
        vision_embedding = vision_embedding * tf.cast(tf.expand_dims(frame_num, -1) > 0, vision_embedding.dtype)
    ...
    # TODO optimizer need to know total stpes
    ...
    def get_loss(self):
        loss_obj = tf.keras.losses.BinaryCrossentropy(reduction=tf.keras.losses.Reduction.NONE, from_logits=False)
