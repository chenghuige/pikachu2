...
class Model(mt.Model):
  ...
  def __init__(self, **kwargs):
    ...
    if not FLAGS.words_bert:
      # better with continue pretrain
      if FLAGS.continue_pretrain: 
        ...
        if os.path.exists(f'{continue_pretrain}/dense'):
          self.vision_dense = mt.load_dense(f'{continue_pretrain}/dense', name='vision')
  ...
  def build(self, input_shape):  
    ...
    if FLAGS.words_bert:
      ...
      ic(pretrain)
      self.bert = TFAutoModelForMaskedLM.from_pretrained(pretrain)
      if os.path.exists(f'{pretrain}/dense'):
        self.vision_dense = mt.load_dense(f'{pretrain}/dense', name='vision')
        ic(self.bert, self.vision_dense)
    ...
    if self.vision_dense is None:
      self.vision_dense =  tf.keras.layers.Dense(FLAGS.bert_size, name='vision')
    ...
    if FLAGS.mlmloss_rate > 0:
      self.mlm =  mt.pretrain.bert.Model(bert=self.bert, dense=self.vision_dense, tokenizer=self.tokenizer)
  ...
  def forward(self, inputs, return_emb=False, index=None, training=None):
    ...
    ids = inputs['input_ids'] if 'input_ids' in inputs else inputs['title_ids']
    vision_attention_mask = tf.sequence_mask(num_frames, FLAGS.max_frames, dtype=ids.dtype)
    ...
    if not FLAGS.merge_vision:
      ...
      if not FLAGS.merge_vision_after_vlad:
        ...
        attention_mask = tf.concat([attention_mask, vision_attention_mask], 1)
      else:
        vision_embs = self.vision_dense(vision_embs)
        vision_attention_mask = tf.ones_like(vision_embs[:,:,0], dtype=attention_mask.dtype)
        attention_mask = tf.concat([attention_mask, vision_attention_mask], 1)
    ...
    if FLAGS.mlmloss_rate > 0:
      self.mlm_inputs = {}
      self.mlm_inputs['input_ids'] = inputs['title_ids']
      if not 'input_ids' in inputs:
        self.mlm_inputs['input_ids'] = inputs['title_ids'] if not FLAGS.words_bert else inputs['title_word_ids']
      else:
        if FLAGS.word:
          self.mlm_inputs['input_ids'] = inputs['word_ids']
        else:
          self.mlm_inputs['input_ids'] = inputs['input_ids']
      self.mlm_inputs['embs'] = vision_feats
      self.mlm_inputs['embs_attention_mask'] = vision_attention_mask
      self.mlm_pred = self.mlm(self.mlm_inputs, training=training)
  ...
  def pointwise_loss(self, y_true, y_pred, label_strategy=None, loss_fn_name=None):
    ...
    if FLAGS.normalloss_rate > 0:
      ...
      loss = mt.reduce_over(loss)
      loss *= FLAGS.normalloss_rate 
      self.scalar('loss/normal', loss)
      loss_ += loss
    
    if FLAGS.mlmloss_rate > 0:
      loss_fn = self.mlm.get_loss_fn()
      loss = loss_fn(self.mlm_inputs['input_ids'], self.mlm_pred)
      loss *= FLAGS.loss_scale
      loss *= FLAGS.mlmloss_rate 
      self.scalar('loss/mlm', loss)
      loss_ += loss
