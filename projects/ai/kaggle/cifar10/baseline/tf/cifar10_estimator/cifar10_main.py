...
def get_model_fn(num_gpus, variable_strategy, num_workers):
  ...
  def _resnet_model_fn(features, labels, mode, params):
    ...
    for i in range(num_devices):
      ...
      with tf.compat.v1.variable_scope('resnet', reuse=bool(i != 0)):
        with tf.compat.v1.name_scope('tower_%d' % i) as name_scope:
    ...
    gradvars = []
    with tf.compat.v1.name_scope('gradient_averaging'):
      all_grads = {}
    ...
    with tf.device(consolidation_device):
      ...
      loss = tf.reduce_mean(input_tensor=tower_losses, name='loss')
      ...
      train_op = [
          optimizer.apply_gradients(
              gradvars, global_step=tf.compat.v1.train.get_global_step())
      ]
      ...
      metrics = {
          'accuracy':
              tf.compat.v1.metrics.accuracy(stacked_labels, predictions['classes'])
      }
...
def _tower_fn(is_training, weight_decay, feature, label, data_format,
  ...
  tower_loss = tf.reduce_mean(input_tensor=tower_loss)
  ...
  tower_grad = tf.gradients(ys=tower_loss, xs=model_params)
