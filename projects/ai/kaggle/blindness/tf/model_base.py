...
class ResNet(object):
  ...
  def _residual_v1(self,
    ...
    with tf.compat.v1.name_scope('residual_v1') as name_scope:
      orig_x = x
      ...
      if in_filter != out_filter:
        ...
        if self._data_format == 'channels_first':
          orig_x = tf.pad(tensor=orig_x, paddings=[[0, 0], [pad, pad], [0, 0], [0, 0]])
        else:
          orig_x = tf.pad(tensor=orig_x, paddings=[[0, 0], [0, 0], [0, 0], [pad, pad]])

      x = self._relu(tf.add(x, orig_x))

      tf.compat.v1.logging.info('image after unit %s: %s', name_scope, x.get_shape())
      return x
  ...
  def _residual_v2(self,
    ...
    with tf.compat.v1.name_scope('residual_v2') as name_scope:
  ...
  def _bottleneck_residual_v2(self,
    ...
    with tf.compat.v1.name_scope('bottle_residual_v2') as name_scope:
      if activate_before_residual:
        x = self._batch_norm(x)
        x = self._relu(x)
        orig_x = x
  ...
  def _conv(self, x, kernel_size, filters, strides, is_atrous=False):
    ...
    if not is_atrous and strides > 1:
      ...
      pad_end = pad - pad_beg
      if self._data_format == 'channels_first':
        x = tf.pad(tensor=x, paddings=[[0, 0], [0, 0], [pad_beg, pad_end], [pad_beg, pad_end]])
      else:
        x = tf.pad(tensor=x, paddings=[[0, 0], [pad_beg, pad_end], [pad_beg, pad_end], [0, 0]])
      padding = 'VALID'
  ...
  def _fully_connected(self, x, out_dim):
    with tf.compat.v1.name_scope('fully_connected') as name_scope:
      x = tf.compat.v1.layers.dense(x, out_dim)

    tf.compat.v1.logging.info('image after unit %s: %s', name_scope, x.get_shape())
    return x

  def _avg_pool(self, x, pool_size, stride):
    with tf.compat.v1.name_scope('avg_pool') as name_scope:
  ...
  def _global_avg_pool(self, x):
    with tf.compat.v1.name_scope('global_avg_pool') as name_scope:
