    ...
    with tf.device('/cpu:0'):
      if not tf.executing_eagerly():
        ...
        if self.data is None:
          ...
          self.data = tf.compat.v1.get_variable('input_%s' % self.subset, dtype=tf.int32, shape=data_npy.shape,trainable=False)
          data_placeholder = tf.compat.v1.placeholder(tf.int32, data_npy.shape)
