    ...
    with ops.name_scope(
      ...
      if cell_state is None:  # first call, return encoder_state
        ...
        if input_text is not None:
          ...
          cell_output_ =  tf.compat.v1.get_variable("cell_output_", [256, vocab_size], dtype=dtypes.float32, initializer=tf.compat.v1.zeros_initializer())
          ...
          cell_output_ = tf.compat.v1.assign(cell_output_, array_ops.zeros([256, vocab_size]))
          indices = melt.batch_values_to_indices(tf.cast(input_text, dtype=tf.int32))
          updates = scores
          #updates = alignments
          cell_output_ = tf.compat.v1.scatter_nd_add(cell_output_, indices, updates)
    ...
    with ops.name_scope(
      ...
      if cell_output is None:
        ...
        if input_text is None:
          ...
          next_input_id = tf.cast(next_input_id, dtype=tf.int32)
...
def _create_attention_score_fn(name,
  ...
  with variable_scope.variable_scope(name, reuse=reuse) as scope:
    ...
    def attention_score_fn(query, keys, values):
      ...
      tf.compat.v1.add_to_collection('attention_scores', scores)

      alignments = nn_ops.softmax(scores)

      tf.compat.v1.add_to_collection('attention_alignments', alignments)
