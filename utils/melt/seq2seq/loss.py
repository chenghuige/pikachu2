...
def exact_predict_loss(logits, targets, mask, num_steps, 
  ...
  def body(i, log_probs, lengths, log_probs_list):
    ...
    if need_softmax:
      ...
      step_log_probs = tf.math.log(tf.maximum(step_logits, 1e-12))
    ...
    step_indices = tf.stack((batch_nums, tf.cast(step_targets, dtype=tf.int32)), axis=1) # shape (batch_size, 2)
    ...
    lengths += tf.cast(step_mask, dtype=tf.int32)
    if not trace_probs:
      masked_log_probs = ()
  ...
  _, log_probs, lengths, log_probs_list = tf.while_loop(cond=condition, body=body, loop_vars=[i, log_probs, lengths, log_probs_list])
    
  log_probs_list = tf.transpose(a=log_probs_list.stack(), perm=[1, 0])
  tf.compat.v1.add_to_collection('seq2seq_logprobs', log_probs_list)

  loss = -log_probs
  if average_across_timesteps:
    loss /= tf.cast(lengths, dtype=tf.float32)
  return loss
...
def sigmoid_loss(logits, targets, mask, num_steps, vocab_size, 
  ...
  labels = tf.cast(melt.nhot(targets, vocab_size), dtype=tf.float32)
  print('labels', labels)
  def body(i, sigmoid_loss, lengths):
    ...
    step_sigmoid_loss = tf.reduce_mean(input_tensor=step_sigmoid_loss, axis=-1)
  ...
  _, sigmoid_loss, lengths = tf.while_loop(cond=condition, body=body, loop_vars=[i, sigmoid_loss, lengths])
