...
class BeamDecoder():
  def __init__(self, input, max_steps, initial_state, beam_size=7, done_token=0,
    ...
    if self.batch_size is None:
      self.batch_size = tf.shape(input=input)[0]
  ...
  def take_step(self, i, prev, state):
    ...
    if self.need_softmax:
      ...
      logprobs = tf.math.log(tf.maximum(output, 1e-12))
    
    if self.num_classes is None:
      self.num_classes = tf.shape(input=logprobs)[1]
    ...
    if self.past_symbols is None:
      ...
      if not self.fast_greedy:
        ...
        done_parent_refs = tf.cast(tf.argmax(input=logprobs_done, axis=1), tf.int32)
        ...
        logprobs_done_max = tf.reduce_max(input_tensor=logprobs_done, axis=1)
      
        if self.length_normalization_factor > 0:
          logprobs_done_max /= i ** self.length_normalization_factor

        #[batch_size, max_len]
        self.finished_beams = tf.compat.v1.where(logprobs_done_max > self.logprobs_finished_beams,
                                       symbols_done,
                                       self.finished_beams)
