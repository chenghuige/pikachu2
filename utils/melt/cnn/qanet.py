...
class QANet(object):
  ...
  def encode(self, inputs, seq_len=None, output_method='all'):
    with tf.compat.v1.variable_scope(self.scope, reuse=tf.compat.v1.AUTO_REUSE):
      inputs = highway(inputs, size=self.num_filters, scope="highway", dropout=self.dropout, reuse=tf.compat.v1.AUTO_REUSE)
      ...
      for i in range(self.num_blocks):
        ...
        outputs, sublayer = self_attention_block(outputs, self.num_filters, seq_len, mask=mask, num_heads=self.num_heads,
            scope="self_attention_layers%d"%i, reuse=tf.compat.v1.AUTO_REUSE, is_training=self.is_training,
