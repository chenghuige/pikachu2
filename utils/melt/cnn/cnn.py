  ...
  def encode(self, seq, seq_len=None, output_method='all'):
    with tf.compat.v1.variable_scope(self.scope, reuse=tf.compat.v1.AUTO_REUSE):
      if self.use_position_encoding:
        ...
        padding = tf.cast(tf.sequence_mask(seq_len), dtype=tf.float32)

        # Set all padding embedding values to 0
        seq *= tf.expand_dims(padding, -1)

        pos_encoding = model_utils.get_position_encoding(
            tf.shape(input=seq)[1], tf.shape(input=seq)[-1])
      ...
      for layer in range(self.num_layers):
        ...
        seq = tf.compat.v1.layers.conv1d(seqs[-1], num_filters, kernel_size=kernel_sizes[layer], padding='same', activation=tf.nn.relu)
