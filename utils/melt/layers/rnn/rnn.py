...
class InitStates(Layer):
  def __init__(self, num_layers, num_units, name='init_fw'):
    ...
    for layer in range(num_layers):
      self.init[layer] = self.add_variable("%s_%d" % (name, layer), [1, num_units], initializer=tf.compat.v1.zeros_initializer())
...
class CudnnRnn(keras.Model):
  def __init__(self,  
    ...
    if isinstance(cell, str):
      if cell == 'gru':
        self.Cell = tf.compat.v1.keras.layers.CuDNNGRU 
        # if tf.test.is_gpu_available():
        #   self.Cell = layers.CuDNNGRU
        # else:
        #   self.Cell = layers.GRU
      elif cell == 'lstm':
        self.Cell = tf.compat.v1.keras.layers.CuDNNLSTM
        # if tf.test.is_gpu_available():
  ...
  def call(self, 
    ...
    if sequence_length is None:
      ...
      sequence_length = tf.ones_like(x[:,0, 0], tf.int32) * len_
    ...
    for layer in range(self.num_layers):
      ...
      if self.recurrent_dropout:
        if mask_fws is not None:
          ...
          if not self.share_dropout:
            mask_fw = dropout(tf.ones_like(x, dtype=tf.float32)[:,:1,:],
      ...
      if mask_bws is not None:
        ...
        if not self.share_dropout:
          ...
          #                           keep_prob=keep_prob, training=training, mode=None)
          mask_bw = dropout(tf.ones_like(x, dtype=tf.float32)[:,:1,:], keep_prob=keep_prob, training=training, mode=None)
        else:
      ...
      inputs_bw = tf.reverse_sequence(
          input=inputs_bw, seq_lengths=sequence_length, seq_axis=1, batch_axis=0)
      ...
      out_bw = tf.reverse_sequence(
          input=out_bw, seq_lengths=sequence_length, seq_axis=1, batch_axis=0)
...
class CudnnRnn2(CudnnRnn):
  ...
  def call(self, 
    ...
    for layer in range(self.num_layers):
      ...
      mask_bw = mask_bws[layer]
      inputs_bw = tf.reverse_sequence(
          input=outputs[-1] * mask_bw, sequence_lengthgths=sequence_length, seq_axis=1, batch_axis=0)
      inputs_bw2 = tf.reverse_sequence(
          input=outputs2[-1] * mask_bw, sequence_lengthgths=sequence_length2, seq_axis=1, batch_axis=0)
