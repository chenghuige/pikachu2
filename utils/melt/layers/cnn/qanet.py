...
# initializer = lambda: tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0,
#                                                              mode=('FAN_AVG').lower(),
#                                                              distribution=("uniform" if True else "truncated_normal"),
#                                                              dtype=tf.float32)
# initializer_relu = lambda: tf.compat.v1.keras.initializers.VarianceScaling(scale=2.0,
#                                                              mode=('FAN_IN').lower(),
#                                                              distribution=("uniform" if False else "truncated_normal"),
#                                                              dtype=tf.float32)
# regularizer = tf.keras.regularizers.l2(l = 0.5 * (3e-7))
...
#     with tf.compat.v1.name_scope(scope, default_name="noam_norm", values=[x]):
#         shape = x.get_shape()
#         ndims = len(shape)
#         return tf.nn.l2_normalize(x, ndims - 1, epsilon=epsilon) * tf.sqrt(tf.cast(shape[-1], dtype=tf.float32))
...
#     with tf.compat.v1.variable_scope(scope, default_name="layer_norm", values=[x], reuse=reuse):
#         scale = tf.compat.v1.get_variable(
#             "layer_norm_scale", [filters], regularizer = regularizer, initializer=tf.compat.v1.ones_initializer())
#         bias = tf.compat.v1.get_variable(
#             "layer_norm_bias", [filters], regularizer = regularizer, initializer=tf.compat.v1.zeros_initializer())
...
#             H = tf.nn.dropout(H, 1 - (1.0 - dropout))
#             x = H * T + x * (1.0 - T)
#         return x

# def layer_dropout(inputs, residual, dropout):
#     pred = tf.random.uniform([]) < dropout
#     return tf.cond(pred=pred, true_fn=lambda: residual, false_fn=lambda: tf.nn.dropout(inputs, 1 - (1.0 - dropout)) + residual)
...
#             if (i) % 2 == 0:
#                 outputs = tf.nn.dropout(outputs, 1 - (1.0 - dropout))
...
#         return combine_last_two_dimensions(tf.transpose(a=x,perm=[0,2,1,3]))
...
#         kernel_ = tf.compat.v1.get_variable("kernel_",
#                         filter_shape,
#                         dtype = tf.float32,
...
#     with tf.compat.v1.variable_scope(scope, reuse = reuse):
#         shapes = inputs.shape.as_list()
#         depthwise_filter = tf.compat.v1.get_variable("depthwise_filter",
#                                         (kernel_size[0], kernel_size[1], shapes[-1], 1),
#                                         dtype = tf.float32,
#                                         regularizer=regularizer,
#                                         initializer = initializer_relu())
#         pointwise_filter = tf.compat.v1.get_variable("pointwise_filter",
#                                         (1,1,shapes[-1],num_filters),
#                                         dtype = tf.float32,
#                                         regularizer=regularizer,
#                                         initializer = initializer_relu())
#         outputs = tf.nn.separable_conv2d(input=inputs,
...
#     ret = tf.reshape(x, tf.concat([tf.shape(input=x)[:-1], [n, -1]], 0))
#     ret.set_shape(new_shape)
#     return tf.transpose(a=ret,perm=[0,2,1,3])
...
#     A Tensor.
#     """
#     with tf.compat.v1.variable_scope(scope, default_name="dot_product_attention", reuse = reuse):
...
#         weights = tf.nn.dropout(weights, 1 - (1.0 - dropout))
...
#     ret = tf.reshape(x, tf.concat([tf.shape(input=x)[:-2], [-1]], 0))
#     ret.set_shape(new_shape)
#     return ret
...
#     signal = tf.pad(tensor=signal, paddings=[[0, 0], [0, tf.math.mod(channels, 2)]])
...
#         x_shape = []
#         for i, s in zip(x.get_shape().as_list(), tf.unstack(tf.shape(input=x))):
#             if i is not None:
#                 x_shape.append(i)
#             else:
#                 x_shape.append(s)
...
#         y_shape = []
#         for i, s in zip(y.get_shape().as_list(), tf.unstack(tf.shape(input=y))):
#             if i is not None:
#                 y_shape.append(i)
#             else:
#                 y_shape.append(s)
...
#         yt = tf.reshape(tf.transpose(a=y, perm=y_permute_dim), [y_shape[-2], -1])
...
#     if x_ndim > y_ndim:
#         diff = x_ndim - y_ndim
#         y = tf.reshape(y, tf.concat([tf.shape(input=y), [1] * (diff)], axis=0))
#     elif y_ndim > x_ndim:
#         diff = y_ndim - x_ndim
#         x = tf.reshape(x, tf.concat([tf.shape(input=x), [1] * (diff)], axis=0))
#     else:
#         diff = 0
#     if ndim(x) == 2 and ndim(y) == 2:
#         if axes[0] == axes[1]:
#             out = tf.reduce_sum(input_tensor=tf.multiply(x, y), axis=axes[0])
#         else:
#             out = tf.reduce_sum(input_tensor=tf.multiply(tf.transpose(a=x, perm=[1, 0]), y), axis=axes[1])
#     else:
#         if axes is not None:
...
#     dtype = args[0].dtype
#     droped_args = [tf.nn.dropout(arg, 1 - (input_keep_prob)) for arg in args]
#     with tf.compat.v1.variable_scope(scope):
#         weights4arg0 = tf.compat.v1.get_variable(
#             "linear_kernel4arg0", [arg_size, 1],
#             dtype=dtype,
#             regularizer=regularizer,
#             initializer=kernel_initializer)
#         weights4arg1 = tf.compat.v1.get_variable(
#             "linear_kernel4arg1", [arg_size, 1],
#             dtype=dtype,
#             regularizer=regularizer,
#             initializer=kernel_initializer)
#         weights4mlu = tf.compat.v1.get_variable(
...
#         subres1 = tf.tile(tf.transpose(a=dot(droped_args[1], weights4arg1), perm=(0, 2, 1)), [1, c_maxlen, 1])
#         subres2 = batch_dot(droped_args[0] * weights4mlu, tf.transpose(a=droped_args[1], perm=(0, 2, 1)))
...
#         flat_args = [tf.nn.dropout(arg, 1 - (input_keep_prob)) for arg in flat_args]
...
#     left = reduce(mul, [fixed_shape[i] or tf.shape(input=tensor)[i] for i in range(start)])
#     out_shape = [left] + [fixed_shape[i] or tf.shape(input=tensor)[i] for i in range(start, len(fixed_shape))]
...
#     pre_shape = [ref_shape[i] or tf.shape(input=ref)[i] for i in range(ref_stop)]
#     keep_shape = [tensor_shape[i] or tf.shape(input=tensor)[i] for i in range(tensor_start, len(tensor_shape))]
...
#   # Now the computation.
#   with tf.compat.v1.variable_scope(scope, reuse = reuse) as outer_scope:
...
#     with tf.compat.v1.variable_scope('qanet', reuse=tf.compat.v1.AUTO_REUSE):
#       dropout = self.dropout if training else 0.
#       inputs = highway(inputs, size=self.num_filters, scope="highway", dropout=dropout, reuse=tf.compat.v1.AUTO_REUSE)
#       if self.input_projection:
#         inputs = conv(inputs, self.num_filters, name = "input_projection", reuse=tf.compat.v1.AUTO_REUSE)
#       outputs = inputs
#       sublayer = 1
...
#               seq_len=seq_len, scope="encoder_block_%d"%i, reuse=tf.compat.v1.AUTO_REUSE, bias=self.bias,
#               dropout=dropout, sublayers = (sublayer, total_sublayers))
#         mask = tf.sequence_mask(seq_len)
#         outputs, sublayer = self_attention_block(outputs, self.num_filters, seq_len, mask=mask, num_heads=self.num_heads,
#             scope="self_attention_layers%d"%i, reuse=tf.compat.v1.AUTO_REUSE, training=training,
