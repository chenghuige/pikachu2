...
from melt.flow.flow import tf_flow, dist_flow
...
def train_flow(ops, 
  ...
  if save_model:
    logging.debug('Will save model to %s' % model_dir)
  else:
    no_log = True
    logging.debug('Will not save model, only read model from %s if exists' % model_dir)
  
  if not log_dir and not no_log:
    log_dir = gezi.get_dir(model_dir)
  if log_dir:
    logging.debug('Will save log to %s' % log_dir)
    ...
    logging.debug('Will not save log')

  def train_once_(sess, step, is_start=False, fixed_step=None, 
                  num_epochs=None, model_path=None, valid_interval_epochs=1., timer_=None, use_horovod=False):
    train_once(sess, 
               step, 
               ops, 
               names, 
