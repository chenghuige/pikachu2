...
def init_weights(shape, stddev=0.01, name=None):
  return tf.Variable(tf.random.normal(shape, stddev=stddev), name=name)
...
def init_weights_uniform(shape, minval=0, maxval=None, name=None):
  return tf.Variable(tf.random.uniform(shape, minval, maxval), name=name)
...
def get_weights(name, shape, minval=-0.08, maxval=0.08, trainable=True):
  return tf.compat.v1.get_variable(name, shape, initializer=tf.compat.v1.random_uniform_initializer(minval, maxval), trainable=trainable)

def get_weights_truncated(name, shape, stddev=1.0, trainable=True):
  return tf.compat.v1.get_variable(name, shape, initializer=tf.compat.v1.truncated_normal_initializer(stddev=stddev), trainable=trainable)

def get_weights_random(name, shape, stddev=1.0, trainable=True):
  return tf.compat.v1.get_variable(name, shape, initializer=tf.compat.v1.random_normal_initializer(stddev=stddev), trainable=trainable)
...
def get_bias(name, shape, val=0.1, trainable=True):
  if not isinstance(shape, (list, tuple)):
    shape = [shape]
  return tf.compat.v1.get_variable(name, shape, initializer=tf.compat.v1.constant_initializer(val), trainable=trainable)