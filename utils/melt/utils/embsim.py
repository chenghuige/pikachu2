...
class EmbeddingSim:
  def __init__(self, emb=None, fixed_emb=None, 
              ...
              sess=None):
    self._sess = sess or tf.compat.v1.InteractiveSession()
    ...
    if model_dir is None:
      ...
      embedding_saver = tf.compat.v1.train.Saver({name: emb})
  ...
  def fixed_sim(self, left_ids, right_ids):
    left_feature = tf.nn.embedding_lookup(params=self._fixed_emb, ids=left_ids)
    right_feature = tf.nn.embedding_lookup(params=self._fixed_emb, ids=right_ids)
    return melt.dot(left_feature, right_feature)
