  ...
  def __init__(self, 
    ...
    if isinstance(weight_op, str):
      try:
        ...
        self.weight_op = tf.compat.v1.get_collection(weight_op)[-1]
        self.name = weight_op
      except Exception:
        raise 'TODO..'
  ...
  def add(self, score):
    ...
    if not self.score or self.cmp(score, self.score):
      ...
      if self.patience >= self.max_patience:
        ...
        if not tf.executing_eagerly():
          self.sess.run(tf.compat.v1.assign(self.weight_op, tf.constant(weight, dtype=tf.float32)))
        else:
        ...
        if 'learning_rate' in self.name:
          if not tf.executing_eagerly():
            ...
            tf.compat.v1.get_collection('learning_rate')[-1].assign(tf.compat.v1.get_collection('learning_rate')[-1] * decay)
...
class WeightsDecay(object):
  def __init__(self, 
    ...
    if isinstance(weights_op, str):
      try:
        self.weights_op = tf.compat.v1.get_collection(weights_op)[-1]
      except Exception:
    ...
    for i, score in enumerate(scores):
      if self.scores is None or self.cmp(score, self.scores[i]):
        ...
        if self.patience[i] >= self.max_patience:
          ...
          if not tf.executing_eagerly():
            self.sess.run(tf.compat.v1.assign(self.weights_op, tf.constant(weights_, dtype=tf.float32)))
          else:
